    ...
    return (
        ...
        {/* <div className="w-full fixed z-[999] px-20 py-8 font-['Neue Montreal'] flex justify-between items-center">
            ...
            <div className={`links hidden lg:flex gap-10`}>
                {["Services", "Our work", "About us", "Insights", "Contact"].map((elem, index) => (
                    <a key={index} className={`text-lg font-light ${show === true && "ml-0"} ${index === 4 && "ml-32"} `}>{elem}</a>
                ))}
            </div>
            <div className='lg:hidden'>
                <button className='transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300' onClick={handle}>
                     <img src={menu} height="50px" className='rounded-[10%] cursor-pointer'  width="50px"/>
                </button>
                   
            </div>
            
        </div>
        <div className={`links -top-[300px] trans ${show === true ? "flex trans items-center px-20 py-8 bg-orange-300 top-0 flex-col gap-1 pt-[100px]":"hidden trans"} lg:hidden gap-10`}>
                {["Services", "Our work", "About us", "Insights", "Contact"].map((elem, index) => (
                    <a key={index} href='#' className={`text-lg font-light ${show === true && "ml-0 font-bold"} ${index === 4 && "ml-32"} `}>{elem}</a>
                ))}
            </div> */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            ...
            <div className="hidden trans md:block">
      ...
      <div className={`${isOpen ? 'block trans' : 'hidden'} md:hidden trans`}>
